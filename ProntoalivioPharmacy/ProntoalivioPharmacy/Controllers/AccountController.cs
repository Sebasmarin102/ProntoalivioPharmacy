...
namespace ProntoalivioPharmacy.Controllers
{
    public class AccountController : Controller
    {
        ...
        public AccountController(IUserHelper userHelper, DataContext context, ICombosHelper combosHelper,
            IBlobHelper blobHelper, IMailHelper mailHelper, IOrdersHelper ordersHelper, IFlashMessage flashMessage)
        {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    _flashMessage.Danger("Ha superado el máximo número de intentos, su cuenta está bloqueada, " +
                        "intente de nuevo en 5 minutos.");
                }
                else if (result.IsNotAllowed)
                {
                    _flashMessage.Danger("El usuario no ha sido habilitado, debes de seguir las instrucciones enviadas al " +
                        "correo para poder habilitarlo.");
                }
                else
                {
