        ...
        private async Task<Response> CheckInventoryAsync(ShowCartViewModel model)
        {
            ...
            foreach (TemporalSale item in model.TemporalSales)
            {
                ...
                if (product == null)
                {
                    ...
                    response.Message = $"El medicamento/producto {item.Product.Name}, ya no está disponible";
                    return response;
                }
                if (product.Stock < item.Quantity)
                {
                    response.IsSuccess = false;
                    response.Message = $"Lo sentimos no tenemos existencias suficientes del medicamento/producto {item.Product.Name}, para tomar su pedido. Por favor disminuir la cantidad o sustituirlo por otro.";
                    return response;
                }
            }
            return response;
